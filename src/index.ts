...
async function run(){
    ...
    let baseMessageOptions: discord.WebhookMessageCreateOptions = {};
    if (WEBHOOK_USERNAME !== ''){
    ...
    let mainMessageOptions: discord.WebhookMessageCreateOptions = { ...baseMessageOptions };
    ...
    if (videos !== undefined){
        ...
        let videoMessageOptions: discord.WebhookMessageCreateOptions = { ...baseMessageOptions };
        videoMessageOptions.files = attachments;
        await webhook_client.send(videoMessageOptions);
    }
}

function getChangelogData(text: string, default_author: string = `Unknown`): { authors: string; infos: string; } | null{
    ...
    function extractCLText(text: string): string | null{
    ...
    function extractInfoLines(text: string): string[]{
...
async function getMedia(text: string): Promise<Map<MediaType, MediaData[]>>{
    core.info(`\n`);
    ...
    async function downloadMedia(mediaUrl: string): Promise<MediaData | null>{
    ...
    function sendRequest(fileUrl: string): Promise<{ extension: string; mediaType: MediaType; buffer: Buffer<ArrayBuffer>; }>{
 ...
 */
function removeCLIgnore(text: string){
    const tag = `CLIgnore`;
...
function removeGitComments(text: string){
    const comment_regex = /<!--[\s\S]*?-->|<!--[\s\S]*/gm;

    return text.replaceAll(comment_regex, '').trim();
}

function exit(code: string | number | null | undefined = undefined): never{
    if (fs.existsSync(ATTACHMENT_SAVE_PATH)){
